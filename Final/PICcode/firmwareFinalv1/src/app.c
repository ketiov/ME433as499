...
void APP_Tasks ( void )
{
    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_CHECK_UART_RECEIVE:
            ...
            if( appData.readLength == 16 )
            {
                //reads in series of COM values from camera
                memcpy( &(inputBuffer[0]), &(appData.readBuffer[0]), 16 );
                ...
                //this finds not the angle, but where the line is relative to center
                //and makes the robot go towards where the line generally is
                for(i = 5 ; i < 16; ++i )
                {
                    sum += inputBuffer[i];
                }

                //sends robot straight
                OC1RS = maxPWM1;
                OC2RS = maxPWM2;
                
                
                //checks the COM overall, negative means go left
                if( sum < -30 )
                {
                    // left wheel turns slower
                    OC1RS = maxPWM1/2;
                    OC2RS = maxPWM2;
                }
                else if( sum > 30 )
                {
                    // right wheel turns slower
                    OC1RS = maxPWM1;
                    OC2RS = maxPWM2/2;

                    //OC1RS = TMR2_PERIOD / 20;
                }
                else
                {
                    // both wheels at max
